...
if os.path.isfile('saved_weights.h5'):
    ...
    checkpoint_cb = tf.keras.callbacks.ModelCheckpoint("saved_weights.h5", 
                                                    save_best_only=True,
                                                    monitor='val_loss') # replace 'val_loss' with the metric you care most about

    # Add the callback to the fit function
    model.fit(dataset, epochs=EPOCHS, callbacks=[tensorboard_callback, checkpoint_cb])
...
while True:
    ...
    output = predict(sentence, loaded_model)
    print('Predicted response:', output)
